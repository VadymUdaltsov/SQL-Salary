...
public class ExpensesServiceMySQL implements ExpensesDAO {
    ...
    @Override
    public Expenses getExpensesForMonth(int year, String month, String passport) {
        try {
        ...
        try {
            ...
            while (resultSet.next()) {
                initExpenses(resultSet, expenses, worker);
            }
            return expenses;
    ...
    @Override
    public List<Expenses> getExpensesForYear(int year, String passport) {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
        } catch (ClassNotFoundException e) {
            throw new RuntimeException(e);
    ...
    @Override
    public void addExpensesMonth(Expenses expenses, String passport) {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
        } catch (ClassNotFoundException e) {
            throw new RuntimeException(e);
        ...
        try {
            ...
            PREPARED_STATEMENT = CONNECTION.prepareStatement("INSERT INTO monthly_expenses(worker_passport, year,month_ID, month, food, " +
            ...
            for (Field field : fields) {
                ...
                if (field.getName().equals("worker_passport")) {
                    expenses.setWorker_passport(worker.getPassport());
                    PREPARED_STATEMENT.setString(paramIndex, worker.getPassport());
    ...
    private void initExpenses(ResultSet resultSet, Expenses expenses, Worker worker) throws SQLException {
        expenses.setWorker_passport(worker.getPassport());
